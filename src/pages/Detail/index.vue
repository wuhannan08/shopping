        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(spuSaleAttr, index1) of spuSaleAttrList"
                :key="spuSaleAttr.id"
              >
                ...
                <dd
                  ...
                  @click="saleAttrChecked(index1, index2)"
                >
...
export default {
  ...
  mounted() {
    // 路由切换到详情页，页面挂载就发请求
    this.$store.dispatch("getGoodDetail", this.$route.params.id);
  },
  methods: {
    // 点击修改售卖属性的checked状态
    saleAttrChecked(index1, index2) {
      for (let spuSaleAttrValue of this.spuSaleAttrList[index1]
    ...
    // 加入购物车操作
    addCart() {
      let resultPromise = this.$store.dispatch("addCart", {
      ...
      resultPromise.then(
        // 加入购物车成功，跳到成功的路由
        (result) => {
          sessionStorage.setItem("skuInfo", JSON.stringify(this.skuInfo)); // 将商品存在sessionStorage中
          this.$router.push({
            name: "addCartSuccess",
            query: {
              skuNum: this.skuNum,
